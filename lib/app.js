(function() {
  var ROOT, app, context, express, fs, handler, handler_path, http, http_proxy, listing, liveloader, load_source, path, proxy, register, url, _i, _len, _ref;
  fs = require('fs');
  path = require('path');
  http = require('http');
  url = require('url');
  ...
  ROOT = process.argv[2];
  load_source = function(file, callback) {
  ...
  app.get('*', function(req, res, next) {
    ...
    return load_source(file, function(source) {
      var resource;
      if (source) {
        req.file = source;
        return next();
  ...
  register = function(handler, app) {
    return app.get(handler.match, function(req, res) {
      ...
      return handler.compiler(req.file, variables, function(output) {
        return res[dispatch](output);
      });
    });
  };
